...
let oppArr = huskyGames2016.map(extractOpponent);
...
lost.forEach(function (line) {
  console.log(line.opponent + " at " + line.home + ", " + line.opponent_score + " to " + line.home_score);
});
...
let atleastOneFumble = huskyGames2016.filter(function (game) {
  if (game.fumbles > 0) {
    return true;
  } else {
    return false;
...
console.log(atleastOneFumble.length);
...
function makeCombinedFilter(cb1, cb2){
  let combinedFilter = function(games){
    let callBack1Answer = cb1(games);
    let callBack2Answer = cb2(games);
    return callBack1Answer && callBack2Answer;
  };
